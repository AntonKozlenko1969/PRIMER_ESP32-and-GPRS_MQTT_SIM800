...
// Производит передачу команд в модуль SIM800, и ожидает ответ от модуля.
// сам ответ обрабатывается в основном цикле, эта задача только следит за глобальной переменной comand_OK и не превышением интервала ее ожидания
...
void GPRS_modem_traffic( void * pvParameters ){
  ...
  int _povtor = 0;  //возможное количество повторов текущей команды, в случае превышения интервала ожидания
...
uint8_t command_type =0; //тип отправленной в модем команды 
                         // 6 - reset sim800
                         // 7 - настроить GPRS соединение
                         // 8 - подключить TCP/IP соединение и отправить блок данных MQTT серверу
                         // 9 - удалить все SMS
                         // 11 - тестовый опрос модема раз в 5 минут
                         // 20 - команда начальной отправки СМС
                         // 30 - одиночная текстовая команда для модема

int8_t _step = 0; //текущий шаг в процедуре GPRS_traffic 
  ...
  if (!_AT_ret && _step !=0) // если предидущая команда завершилась неудачно (превышен интервал и количество повторов исчерпано) 
      ...
      else modemOK = false; //иначе скорее всего сбой модема, создать условия для его рестарта
         }
...
// *****   Блок выполнения АТ команды с повтором в случае таймаута и выполнение второй части команды если логика того требует *******
 if (command_type != 0) {
// только при отправке текста СМС или при отправке данных, после получения приглашения > не добавлять AT в начало команды и '\r' в конце
  ...
  if (_step == 13 && (flag_modem_resp == 6 || flag_modem_resp == 8)) { //надо отправить модуль данные после приглашения на ввод '>'
     ...
     //сбросить флаг только при признаке 8 (нужен чтобы отследить приглашение на ввод данных ">" при отправке в TCP соединение)
      flag_modem_resp = 0;  
       }
     }
